...
// Fetch the latest delivery charge
$charge_query = mysqli_query($conn, "SELECT * FROM delivery_charges ORDER BY id DESC LIMIT 1");
$current_charge = mysqli_fetch_assoc($charge_query);
$delivery_charge = $current_charge ? $current_charge['charge_amount'] : 0;
...
if (isset($_POST['download_voucher'])) {
    ...
    echo "<script>alert('Order placed! Kindly pay your 1st voucher to proceed to the next.'); window.location.href='order-now.php';</script>";
    exit;
}
...
<body>
  <div class="notification-bubble">
    Current Delivery Charge: Rs. <?= number_format($delivery_charge, 2) ?>
  </div>
  ...
  <div style="display: flex;">
    ...
    <div class="main">
      ...
      <?php } else { ?>
        ...
        <?php if ($show_cancel) { ?>
          <form method="POST">
            <button type="submit" name="cancel_order" class="btn btn-danger">Cancel Your Order</button>
          </form>
        <?php } ?>
      <?php } ?>
    </div>
